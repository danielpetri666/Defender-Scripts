<#
.SYNOPSIS
    Retrieves ASR audit events and exports them to an Excel file.
.DESCRIPTION
...
$ExportFile = Join-Path $ExportPath "$(Get-Date -Format 'yyyy-MM-dd HHmm') - ASR Audit Report.xlsx"
...
Write-Host ""
Write-Host "========== ASR AUDIT REPORT ==========" -ForegroundColor Magenta
...
foreach ($Rule in $AsrRules) {
    ...
    foreach ($DR in $DetailResults) {
        $RuleDetails += [PSCustomObject]@{
            ...
            IsAudit                                     = ($DR.ActionType -like "*Audited*")
        }
    }

    $totalAudits = $RuleDetails.Count
    $uniqueDeviceCount = @(
        $RuleDetails |
    ...
    $Summary += [PSCustomObject]@{
        RuleName            = $RuleName
        RuleId              = $RuleId
        Total_Audit_Count   = $totalAudits
        Devices             = $uniqueDeviceCount
        MachineGroups       = $machineGroupCount
        ZeroAuditLastPeriod = ($totalAudits -eq 0)
    ...
    Write-Log -Level OK -Message "Done. Audits: $totalAudits | Devices: $uniqueDeviceCount | Status: $($DetailQueryResult.Status) | Requests: $($DetailQueryResult.TotalRequests) | Failed: $($DetailQueryResult.FailedRequests)" -Indent 1
...
$Summary |
Sort-Object -Property @{ Expression = 'Total_Audit_Count'; Descending = $true }, RuleName |
